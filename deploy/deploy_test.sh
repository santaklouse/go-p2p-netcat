...
cosign_stub="${test_root}/cosign"
cosign_log="${test_root}/cosign.log"
mkdir -p "${package_dir}"

printf '%s\n' \
	'#!/usr/bin/env sh' \
	'printf "%s\n" "$*" >>"${P2PNC_TEST_COSIGN_LOG}"' \
	'exit "${P2PNC_TEST_COSIGN_EXIT:-0}"' >"${cosign_stub}"
chmod 0755 "${cosign_stub}"
...
P2PNC_RELEASE_BASE="file://${release_dir}" \
	P2PNC_COSIGN="${cosign_stub}" \
	P2PNC_TEST_COSIGN_LOG="${cosign_log}" \
...
grep -q -- 'verify-blob.*SHA256SUMS.*--bundle.*SHA256SUMS.sigstore.json' "${cosign_log}"
grep -q -- '--certificate-identity-regexp.*release-main\\.yml@refs/' "${cosign_log}"
...
if P2PNC_RELEASE_BASE="file://${release_dir}" \
	P2PNC_COSIGN="${cosign_stub}" \
	P2PNC_TEST_COSIGN_LOG="${cosign_log}" \
	P2PNC_TEST_COSIGN_EXIT=1 \
	P2PNC_INSTALL_DIR="${install_dir}" \
	P2PNC_OS=linux \
	P2PNC_ARCH=amd64 \
	P2PNC_NO_SUDO=1 \
	bash "$(dirname "$0")/deploy.sh" >/dev/null 2>&1; then
	printf 'invalid Sigstore bundle unexpectedly passed\n' >&2
	exit 1
fi
