...
func New(peerID peer.ID, service uint16, relayHints []ma.Multiaddr, expiresAt *uint64) (*Token, error) {
	if service == 0 {
		return nil, errors.New("логический порт должен быть от 1 до 65535")
	}
	token := &Token{
	...
	if _, err := rand.Read(token.Secret[:]); err != nil {
		return nil, fmt.Errorf("создать pairing secret: %w", err)
	}
...
func (t *Token) Validate(now time.Time) error {
	if t == nil {
		return errors.New("pairing token не задан")
	}
	if t.Version != TokenVersion {
		return fmt.Errorf("неподдерживаемая версия pairing token: %d", t.Version)
	}
	if _, err := peer.Decode(t.PeerID.String()); err != nil {
		return fmt.Errorf("некорректный PeerId в pairing token: %w", err)
	}
	if t.Service == 0 {
		return errors.New("логический порт pairing token должен быть от 1 до 65535")
	}
	if len(t.RelayHints) > MaxRelayHints {
		return fmt.Errorf("pairing token поддерживает не более %d relay hints", MaxRelayHints)
	}
	seen := make(map[string]struct{}, len(t.RelayHints))
	for _, hint := range t.RelayHints {
		if hint == nil || len(hint.String()) > 2048 {
			return errors.New("некорректный relay hint в pairing token")
		}
		info, err := peer.AddrInfoFromP2pAddr(hint)
		if err != nil || info.ID == "" {
			return fmt.Errorf("relay hint должен содержать /p2p/PeerId: %s", hint)
		}
		if _, ok := seen[hint.String()]; ok {
			return fmt.Errorf("relay hints должны быть уникальными: %s", hint)
		}
		seen[hint.String()] = struct{}{}
	}
	if t.ExpiresAt != nil {
		if *t.ExpiresAt == 0 {
			return errors.New("expiration pairing token должна быть положительным Unix timestamp")
		}
		if !now.IsZero() && uint64(now.Unix()) > *t.ExpiresAt {
			return fmt.Errorf("pairing token истёк в Unix time %d", *t.ExpiresAt)
		}
	}
	return nil
}
...
func (t *Token) Encode() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("кодировать pairing token: %w", err)
	}
...
func Decode(text string) (*Token, error) {
	...
	if !strings.HasPrefix(text, TokenPrefix) {
		return nil, fmt.Errorf("pairing token должен начинаться с %s", TokenPrefix)
	}
	payload, err := base64.RawURLEncoding.Strict().DecodeString(strings.TrimPrefix(text, TokenPrefix))
	if err != nil {
		return nil, fmt.Errorf("некорректные base64url-данные pairing token: %w", err)
	}
	if len(payload) == 0 || len(payload) > MaxTokenSize {
		return nil, fmt.Errorf("pairing token должен содержать 1..%d байт", MaxTokenSize)
	}
	var raw map[uint64]cbor.RawMessage
	if err := strict.Unmarshal(payload, &raw); err != nil {
		return nil, fmt.Errorf("некорректный deterministic CBOR pairing token: %w", err)
	}
	for key := range raw {
		if key > 5 {
			return nil, fmt.Errorf("неизвестное поле pairing token: %d", key)
		}
	}
	for _, key := range []uint64{0, 1, 2, 3, 4} {
		if _, ok := raw[key]; !ok {
			return nil, fmt.Errorf("отсутствует поле pairing token: %d", key)
		}
	}
	...
	if err := strict.Unmarshal(raw[0], &version); err != nil {
		return nil, fmt.Errorf("версия pairing token: %w", err)
	}
	...
	if err := strict.Unmarshal(raw[2], &service); err != nil || service == 0 || service > 65535 {
		return nil, errors.New("логический порт pairing token должен быть от 1 до 65535")
	}
	if err := strict.Unmarshal(raw[3], &secret); err != nil || len(secret) != SecretSize {
		return nil, fmt.Errorf("pairing secret должен содержать ровно %d байта", SecretSize)
	}
	...
	if len(hints) > MaxRelayHints {
		return nil, fmt.Errorf("pairing token поддерживает не более %d relay hints", MaxRelayHints)
	}
	peerID, err := peer.Decode(peerText)
	if err != nil {
		return nil, fmt.Errorf("некорректный PeerId в pairing token: %w", err)
	}
	...
	for _, value := range hints {
		...
		if err != nil {
			return nil, fmt.Errorf("некорректный relay hint %q: %w", value, err)
		}
	...
	if reencoded != expected {
		return nil, errors.New("CBOR pairing token не соответствует deterministic RFC 8949")
	}
	return token, nil
}
...
func (t *Token) DeriveKey(purpose string) ([32]byte, error) {
	...
	default:
		return result, fmt.Errorf("неподдерживаемое назначение pairing key: %s", purpose)
	}
...
func (t *Token) RendezvousID(purpose string, epoch uint64) (string, error) {
	...
	default:
		return "", fmt.Errorf("неподдерживаемое rendezvous-назначение: %s", purpose)
	}
...
func ProviderCID(rendezvousID string) (cid.Cid, error) {
	...
	if err != nil || len(decoded) != 32 || base64.RawURLEncoding.EncodeToString(decoded) != rendezvousID {
		return cid.Undef, errors.New("некорректный rendezvous identifier")
	}
...
func (t *Token) Seal(purpose string, plaintext, additionalData, nonce []byte) ([]byte, error) {
	...
	if len(nonce) != 12 {
		return nil, errors.New("AES-GCM nonce должен содержать ровно 12 байт")
	}
...
func (t *Token) Open(purpose string, envelope, additionalData []byte) ([]byte, error) {
	...
	if err := strict.Unmarshal(envelope, &fields); err != nil {
		return nil, fmt.Errorf("некорректный AES-GCM envelope: %w", err)
	}
	if len(fields) != 3 || fields[0] == nil || fields[1] == nil || fields[2] == nil {
		return nil, errors.New("AES-GCM envelope содержит неизвестные или отсутствующие поля")
	}
	var version uint64
	var nonce, ciphertext []byte
	if strict.Unmarshal(fields[0], &version) != nil || version != 1 {
		return nil, errors.New("неподдерживаемая версия AES-GCM envelope")
	}
	if strict.Unmarshal(fields[1], &nonce) != nil || len(nonce) != 12 {
		return nil, errors.New("AES-GCM nonce должен содержать ровно 12 байт")
	}
	if strict.Unmarshal(fields[2], &ciphertext) != nil || len(ciphertext) < 16 {
		return nil, errors.New("AES-GCM ciphertext короче authentication tag")
	}
	key, err := t.DeriveKey(purpose)
	if err != nil {
		return nil, err
	...
	if err != nil {
		return nil, errors.New("проверка подлинности pairing payload не прошла")
	}
	reencoded, err := canonical.Marshal(map[uint64]any{0: version, 1: nonce, 2: ciphertext})
	if err != nil || !bytes.Equal(reencoded, envelope) {
		return nil, errors.New("AES-GCM envelope не соответствует deterministic RFC 8949")
	}
